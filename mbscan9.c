...
{
unsigned int mbaddress,mbamount;
...
//                 mbamount = (mbreceived_data[4] *256) +mbreceived_data[5];      //requested amount
                 ...
                 if (mbaddress+(mbamount) > 57 || mbamount >16)
                    {
...
//        error handling;
        if (error_code !=0)
            ...
            //todo : error handling code here
                mb_dir =1;
                mbtransmit_data[1] = mbtransmit_data[1] | 0x80;     //set highest bit to indicate exception
...
switch (input[mux_scan])
    ...
    case 1: process_value[mux_scan] = linearise_p(adc_value,cal_zero[mux_scan],cal_span[mux_scan])/10;
            ...
            max_val = 600;
            break;
    case 2: process_value[mux_scan] = linearise_tc(adc_value,cal_zero[mux_scan],cal_span[mux_scan],15,table_j,5000);
            min_val =0;
            max_val = 700;
            break;
    case 3: process_value[mux_scan] = linearise_tc(adc_value,cal_zero[mux_scan],cal_span[mux_scan],28,table_k,5000);
            min_val =0;
            max_val = 1300;
            break;                           
    case 4: process_value[mux_scan] = linearise_tc(adc_value,cal_zero[mux_scan],cal_span[mux_scan],36,table_r,50000);
            min_val =0;
            max_val = 1700;
            break;
    case 5: process_value[mux_scan] = linearise_tc(adc_value,cal_zero[mux_scan],cal_span[mux_scan],36,table_s,50000);
            min_val =0;
            max_val = 1700;
            break;
    case 6: process_value[mux_scan] = linearise_tc(adc_value,cal_zero[mux_scan],cal_span[mux_scan],8,table_t,5000);
            min_val =0;
            max_val = 350;
            break;
    case 7: process_value[mux_scan] = linearise_volt(adc_value,cal_zero[mux_scan],cal_span[mux_scan],rhigh[mux_scan],rlow[mux_scan]);
            min_val =rlow[mux_scan] - (rlow[mux_scan]*20/100);
            max_val =rhigh[mux_scan]+ (rhigh[mux_scan]*20/100);
            break;  
    case 8: process_value[mux_scan] = linearise_420(adc_value,cal_zero[mux_scan],cal_span[mux_scan],rhigh[mux_scan],rlow[mux_scan]);
             min_val =rlow[mux_scan] - (rlow[mux_scan]*20/100);
            max_val =rhigh[mux_scan]+ (rhigh[mux_scan]*20/100);
            break;
    }   
