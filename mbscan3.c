...
unsigned char segment_table[]= {0x84,0xf5,0xc2,0xc1,0xb1,0x89,0x88,0xe5,0x80,0x81,0xa0,0x98,0x8e,0xd0,0x8a,0xaa,0x8c,0xb0,0xd5,0xa8,0x9e,0xe8,0xf8,0xd8,0xa2,0xfa,0x9a,0xdc,0xcc,0x91,0xfb,0x00,0x01,0xff,0x44,0xf1};
...
short int message_menu[] = {33,16,14,22,33,33,23,05,05,19,01,24,25,30,20,23,25,30,17,01,10,30,20,23,10,30,17,01,33,01,22,24};
//st,id,baud
...
process_value[mux_scan] = linearise_p(adc_value,cal_zero[mux_scan],cal_span[mux_scan]);
...
if(!menu_fl && !cal_fl)
    ...
    if (tsec_fl)
        ...
        tsec_fl =0;
        display_scan_cnt++;
        if (display_scan_cnt >=3) display_scan_cnt =0;
