...
///added to store factor to be divided from process value according to dp
for (i =0;i<8;i++)
    {
    switch (dp[i])
    {  
    case 0: mb_inputdata[count] = 1000;
            break;
    case 1: mb_inputdata[count] = 100;       
            break;
    case 2: mb_inputdata[count] = 10;      
            break;
    default:mb_inputdata[count] = 1;
            break;
    }
    count++;
    }
...
temp1 = ((a - zero_tc)*5000 /(span_tc - zero_tc));    //adc value of 300 deg. is 11075 in table_p
//added to add ambient value in table value 
temp1 = temp1 + (table_t[5] * (long)ambient_val /50);
b = (int)temp1;
//if (b<0)
//{
//b = -b;
...
for (count=0;count <= 12; count++)  
    {
    if (b>table_t[count] && b <= table_t[count+1])    
        {
        number = count;
        break;
        }
...
temp = ((50*(temp1-(float)table_t[number]))/((float)table_t[number+1] - (float)table_t[number]))+ ((long)(number-4) * 50);
true_value = (int) temp;
//if (nfl) true_value = -true_value;
return (true_value);
}
    ...
    if (level ==1)
        ...
        switch (item1)
            ...
            case 2: increment_value(&skip[item2],0,1,0);    //skip
                    break;
            case 3: increment_value(&rlow[item2],-200,300,blink_digit);    //rlow
                    break;
            case 4: increment_value(&rhigh[item2],-200,300,blink_digit);   //rhigh
                    break;
            case 5: increment_value(&alow[item2],-200,300,blink_digit);    //alow
                    break;
            case 6: increment_value(&ahigh[item2],-200,300,blink_digit);   //ahigh
                    break;
...
if (menu_fl &&!cal_fl)
    ...
    if (level ==1)
        {
        item1 --; 
        if (item1==4) item1=2;      //skip r-lo/r-hi
        if (item1<0) item1 =6;
        }
    else if (level ==2)
        {
        switch (item1)
            ...
            case 2: decrement_value(&skip[item2],0,1,0);    //skip
                    break;
            case 3: decrement_value(&rlow[item2],-200,300,blink_digit);    //rlow
                    break;
            case 4: decrement_value(&rhigh[item2],-200,300,blink_digit);   //rhigh
                    break;
            case 5: decrement_value(&alow[item2],-200,300,blink_digit);    //alow
                    break;
            case 6: decrement_value(&ahigh[item2],-200,300,blink_digit);   //ahigh
                    break;
...
{
ambient_val = 30 + (adc_value - cal_zero[7])/22;
}
else
{
ambient_val = 30 - (adc_value - cal_zero[7])/22;
...
switch (input[mux_scan])
    ...
    case 5: process_value[mux_scan] = linearise_tc(adc_value,cal_zero[mux_scan],cal_span[mux_scan],36,table_s,50000)+os[mux_scan];
            ...
            max_val = 1700;
            break;
    case 6: process_value[mux_scan] = linearise_t(adc_value,cal_zero[mux_scan],cal_span[mux_scan])+os[mux_scan];
            min_val =-200;
            max_val = 350;
            break;
...
if(!menu_fl && !cal_fl)
    ...
    if (tsec_fl )   //hold_fl =0 implies scan else hold (toggled in shf key routine)
        ...
        if (skip[display_scan_cnt]!=0 && display_scan_cnt <=7)
        goto bypass;
        tsec_fl =0;
        if (display_scan_cnt >=7) display_scan_cnt =0;
...
switch (count2)
    ...
    case 6: if (menu_fl && !cal_fl && (level ==2))
            ...
            break;
    case 7: if (!menu_fl && !cal_fl && hold_fl)
            {
            PORTA.7 =0;
            }   
            break;               
